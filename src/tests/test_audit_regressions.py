...
def test_minimum_rejects_first_weak_result():
    row = {"retrieval_score": 0.01, "dispositivo": SimpleNamespace(norma_id=1)}
    assert AdaptiveRetriever._select([row], 12, 0.9) == []


def test_scope_still_applies_with_all_statuses():
    row = {
        "dispositivo": SimpleNamespace(
            norma=SimpleNamespace(sapl_url="https://example.com", status="pending")
        )
    }
...
def test_article_reference_extracts_containing_law(kind, number):
    ref = parse_target_reference(
        SimpleNamespace(
            referencia_tipo=kind,
...
def test_lexical_context_has_scores_and_only_used_sources():
    ...
    Dispositivo.objects.create(
        norma=norma, tipo="artigo", numero="1", ordem=1, texto="zoneamento " * 100
    )
...
def test_history_cursor_retrieves_all_messages_once(user):
    ...
    ChatMessage.objects.bulk_create(
        [ChatMessage(session=session, role="user", content=str(i)) for i in range(60)]
    )
    seen = []
    cursor = None
    while True:
...
def test_closing_stream_persists_partial_once(user):
    ...
    request.user = user
    events = iter(
        [{"event": "chunk", "chunk": "Resposta parcial"}, {"event": "done", "answer": "Completa"}]
    )
...
def test_reingestion_preserves_extracted_text():
    ...
    norma = Norma.objects.create(
        ...
        ano=2026,
        status="consolidated",
        texto_original="Art. 1º Texto extraído",
        pdf_url="https://example.com/law.pdf",
    )
    _process_norma_data(
        {
            "id": 99,
