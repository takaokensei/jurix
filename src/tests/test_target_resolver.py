...
NORMA_ALVO = NS(id=1, tipo="Lei", numero="123", ano=2020, ementa="", texto_original="",
                data_publicacao=date(2020, 1, 1), data_vigencia=None)
...
def art(id_, numero, texto="Texto.", pai=None, ordem=None, tipo="artigo"):
    return Disp(id=id_, tipo=tipo, numero=numero, texto=texto, dispositivo_pai_id=pai,
              ordem=ordem or id_, norma=NORMA_ALVO)


def evento(id_, acao, tipo, numero, fonte_id=100, fonte_texto="Fica revogado o art. 5º da Lei nº 123/2020.",
           alvo=None, data=date(2021, 5, 1), norma_num=200):
    norma_fonte = NS(tipo="Lei", numero=str(norma_num), ano=data.year, data_publicacao=data, data_vigencia=data)
    fonte = NS(id=fonte_id, norma=norma_fonte, texto=fonte_texto, ordem=1)
    return NS(id=id_, acao=acao, referencia_tipo=tipo, referencia_numero=numero,
              target_text=f"art. {numero}" if tipo == "artigo" else numero,
              dispositivo_alvo=alvo, dispositivo_fonte=fonte, dispositivo_fonte_id=fonte_id)


DISPS = [art(1, "1º"), art(2, "5º", "Prazo antigo."), art(3, "5º-A"), art(4, "9º"),
...
class TestArticleKey:
    def test_parsing(self):
        assert article_key("5º") == (5, "") and article_key("5º-A") == (5, "A") and article_key("10") == (10, "")
...
class TestResolutionOfPlainArticles:
    ...
    def test_several_plain_articles_in_one_sentence_all_resolve(self):
        ...
        evs = [evento(1, "REVOGA", "artigo", "5º", fonte_texto=text), evento(2, "REVOGA", "artigo", "9º", fonte_texto=text)]
    ...
    def test_article_missing_from_the_target_norma_is_unresolved_with_a_reason(self):
        dispositivo, reason = resolved(resolve_targets([evento(1, "REVOGA", "artigo", "77º")], DISPS), 1)
    ...
    def test_duplicated_numbering_in_the_target_is_ambiguous(self):
        ...
        dispositivo, reason = resolved(resolve_targets([evento(1, "REVOGA", "artigo", "5º")], dup), 1)
    ...
    def test_only_top_level_articles_are_candidates_never_a_paragraph_with_the_same_number(self):
        ...
        assert resolved(resolve_targets([evento(1, "REVOGA", "artigo", "5º")], only_par), 1)[0] is None
...
class TestGuardsAgainstWrongRevocations:
    def test_paragraph_of_an_article_never_revokes_the_whole_article(self):
        ...
        evs = [evento(1, "REVOGA", "artigo", "5º", fonte_texto=text), evento(2, "REVOGA", "paragrafo", "2º", fonte_texto=text)]
        res = resolve_targets(evs, DISPS)
    ...
    def test_inciso_and_alinea_references_are_not_resolved(self):
        ...
        evs = [evento(1, "REVOGA", "artigo", "9º", fonte_texto=text), evento(2, "REVOGA", "inciso", "II", fonte_texto=text)]
    ...
    def test_groups_are_per_source_dispositivo_and_action(self):
        ...
        hier = [evento(1, "REVOGA", "artigo", "5º", fonte_id=100), evento(2, "REVOGA", "paragrafo", "2º", fonte_id=100)]
        plain = [evento(3, "REVOGA", "artigo", "9º", fonte_id=101, fonte_texto="Fica revogado o art. 9º da Lei nº 123/2020.")]
    ...
    def test_the_same_norma_cited_twice_is_not_ambiguous(self):
        ...
        assert resolved(resolve_targets([evento(1, "REVOGA", "artigo", "5º", fonte_texto=text)], DISPS), 1)[0] == 2

    def test_self_reference_without_any_other_norma_is_fine(self):
        text = "Fica revogado o art. 9º desta Lei."
        assert resolved(resolve_targets([evento(1, "REVOGA", "artigo", "9º", fonte_texto=text)], DISPS), 1)[0] == 4
...
class TestEngineAppliesResolvedEvents:
    def test_revocation_is_applied_and_hides_the_articles_subtree(self):
        ...
        assert st["events_applied"] == 1 and st["events_unresolved"] == 0 and st["revoked_count"] == 1
    ...
    def test_hierarchical_reference_is_reported_not_applied(self):
        text = "Fica revogado o § 2º do art. 5º da Lei nº 123/2020."
        eng, out = run_engine([evento(1, "REVOGA", "artigo", "5º", fonte_texto=text),
    ...
    def test_later_alteration_wins_over_an_earlier_one(self):
        e1 = evento(1, "ALTERA", "artigo", "5º", fonte_id=100, data=date(2021, 1, 1), norma_num=200,
                    fonte_texto="Altera o art. 5º da Lei nº 123/2020: “Art. 5º Versão dois.”")
        e2 = evento(2, "ALTERA", "artigo", "5º", fonte_id=101, data=date(2022, 1, 1), norma_num=300,
    ...
    def test_revocation_after_alteration_wins_and_alteration_after_revocation_restores(self):
        alt = evento(1, "ALTERA", "artigo", "5º", fonte_id=100, data=date(2021, 1, 1), norma_num=200,
                     fonte_texto="Altera o art. 5º da Lei nº 123/2020: “Art. 5º Novo.”")
        rev = evento(2, "REVOGA", "artigo", "5º", fonte_id=101, data=date(2022, 1, 1), norma_num=300)
        eng, out = run_engine([alt, rev])
        assert eng.get_statistics()["revoked_count"] == 1 and "Novo." not in out
        eng, out = run_engine([rev, evento(3, "ALTERA", "artigo", "5º", fonte_id=102, data=date(2023, 1, 1), norma_num=400,
    ...
    def test_output_is_deterministic_whatever_the_input_order(self):
        evs = [evento(1, "REVOGA", "artigo", "9º", fonte_id=100), evento(2, "ALTERA", "artigo", "5º", fonte_id=101,
               fonte_texto="Altera o art. 5º da Lei nº 123/2020: “Art. 5º Novo.”", data=date(2022, 1, 1), norma_num=300)]
