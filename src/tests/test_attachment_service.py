...
def test_upload_and_delete_text_attachment(tmp_path, settings, monkeypatch):
    ...
    upload = SimpleUploadedFile(
        "consulta.txt", b"Lei municipal sobre zoneamento", content_type="text/plain"
    )
...
def test_resource_limits_enforced_in_process():
    ...
    assert (
        r1.returncode != 10
    ), f"apply_resource_limits failed to install limits: {r1.stderr.decode()}"
    assert (
        r1.returncode != 20
    ), "Memory limit was not enforced: allocation of 100MB succeeded under 30MB limit"
    assert (
        r1.returncode == 42 or (r1.returncode & 0xFFFFFFFF) == 0xC0000044
    ), f"Unexpected returncode: {r1.returncode}"
    ...
    assert (
        b"CPU_LIMIT_INSTALLED" in r2.stdout
    ), f"Process failed before starting compute loop: {r2.stderr.decode()}"
    assert r2.returncode != 20, "CPU limit was not enforced: compute finished under 1s limit"
    if sys.platform == "win32":
        assert (
            (r2.returncode & 0xFFFFFFFF) == 0xC0000044
