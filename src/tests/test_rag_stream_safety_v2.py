...
class RAGStreamingSafetyTests(SimpleTestCase):
    ...
    def test_streaming_does_not_expose_provisional_chunks(self):
        ...
        source = {"text": "A Lei 1234/2025 exige 3 documentos.", "norma_ref": "Lei 1234/2025", "identifier": "Art. 1", "dispositivo_id": 1}
        fake = Mock()
        ...
        with patch.object(service, "get_relevant_context", return_value=("ctx", [source])):
            with patch.object(service, "_answer_uses_only_sources", return_value=True):
                with patch.object(service, "_ground_answer", return_value={"grounded": True, "strict": True, "score": 1.0, "claims": [], "failed_claims": [], "source_diversity_ok": True}):
