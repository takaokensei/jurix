...
class OllamaService:
    ...
    def __init__(self, base_url: str | None = None, model: str = "nomic-embed-text"):
        ...
        """
        self.base_url = (
            base_url or getattr(settings, "OLLAMA_BASE_URL", "http://host.docker.internal:11434")
        ).rstrip("/")
        self.model = model
        ...
        retries = Retry(
            total=2, backoff_factor=0.3, status_forcelist=[502, 503, 504], raise_on_status=False
        )
    ...
    def generate_embedding(self, text: str, model: str | None = None) -> list[float] | None:
        ...
        try:
            ...
            payload = {"model": model, "input": clean_text}
