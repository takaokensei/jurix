...
def decide_grounding(report: dict[str, Any], source_only: bool) -> RAGPolicyDecision:
    ...
    if score < minimum_score:
        return RAGPolicyDecision(False, "grounding_score_below_threshold", False, report)
    return RAGPolicyDecision(True, "accepted", True, report)


def response_metadata(policy: RAGPolicyDecision) -> dict[str, Any]:
    return {
        "policy": "strict-grounding-v1",
