...
def _parse_hierarchy(text: str, reference_type: str, reference_number: str) -> dict[str, Any]:
    ...
    article_candidates: list[
        tuple[int, tuple[int, int, str, str], list[tuple[int, int, str, str]]]
    ] = []
...
def _resolve_hierarchical(
    ...
    for dispositivo in dispositivos:
        if dispositivo.tipo != ref_type or not _number_equal(
            ref_type, dispositivo.numero, path[ref_type]
        ):
            continue
        ...
        for parent_type in ("paragrafo", "inciso", "alinea", "item"):
            ...
            if nodes and not any(
                _number_equal(parent_type, node.numero, expected) for node in nodes
            ):
                matched = False
        if matched:
...
def resolve_targets(eventos: list, dispositivos: list) -> dict[Any, Resolution]:
    ...
    for e in pending:
        ...
        if spec[0] == "range":
            ...
            groups = [
                ds
                for (number, _suffix), ds in sorted(articles.items())
