...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--norma-id", type=int, default=None, help="Process a specific Norma ID only"
        )
        parser.add_argument(
            "--sync",
            action="store_true",
        ...
        parser.add_argument(
            "--limit", type=int, default=None, help="Maximum number of normas to process"
        )
    ...
    def handle(self, *args, **options):
        ...
        for idx, norma in enumerate(queryset, start=1):
            self.stdout.write(
                f"[{idx}/{total}] Processing Norma #{norma.id} ({norma})...", ending=" "
            )
            try:
                if sync_mode:
        ...
        if sync_mode:
            ...
            self.stdout.write(
                self.style.WARNING("Tasks dispatched asynchronously; inspect worker logs.")
            )
