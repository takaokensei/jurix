...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--norma-id", type=int, default=None, help="Process a specific norma by ID"
        )
    ...
    def handle(self, *args, **options):
        ...
        if norma_id:
            try:
                ...
                raise CommandError(f"Norma with ID={norma_id} not found") from None
        ...
        if process_all:
            ...
            queryset = Norma.objects.filter(Q(status="segmented") | Q(status="entities_extracted"))
            self.stdout.write(
                self.style.WARNING(
