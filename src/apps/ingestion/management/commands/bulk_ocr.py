...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--all", action="store_true", help="Process all normas with pdf_downloaded status"
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--force", action="store_true", help="Reprocess normas that already have OCR completed"
        )
    ...
    def handle(self, *args, **options):
        ...
        if limit:
            ...
            if count > 10:
                self.stdout.write(
                    self.style.WARNING(f"You are about to process OCR for {count} normas.")
                )
        ...
        if total == 0:
            self.stdout.write(self.style.WARNING("No normas found for OCR processing"))
            return

        self.stdout.write(self.style.NOTICE(f"Starting OCR processing for {total} norma(s)..."))
