...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.NOTICE("Testando conexão com API SAPL..."))
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"✓ Conexão bem-sucedida! {len(normas)} normas retornadas.\n")
            )
            ...
            if normas:
                ...
                for norma in normas[:3]:
                    ...
                    tipo = (
                        tipo_dict.get("descricao", "N/A")
                        if isinstance(tipo_dict, dict)
                        else str(tipo_dict)
                    )
            ...
            self.stdout.write(self.style.ERROR(f"✗ Falha no teste: {str(e)}"))
            raise
