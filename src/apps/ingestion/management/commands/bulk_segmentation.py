...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--all", action="store_true", help="Process all normas with ocr_completed status"
        )
        parser.add_argument(
            "--limit", type=int, default=None, help="Limit number of normas to process"
        )
        parser.add_argument(
            "--status",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        if force:
            if process_all:
                ...
                queryset = Norma.objects.exclude(texto_original="").exclude(
                    texto_original__isnull=True
                )
            else:
        ...
        if limit:
            ...
            if count > 10:
                self.stdout.write(
                    self.style.WARNING(f"You are about to process segmentation for {count} normas.")
                )
        ...
        if total == 0:
            self.stdout.write(self.style.WARNING("No normas found for segmentation"))
            return

        self.stdout.write(self.style.NOTICE(f"Starting text segmentation for {total} norma(s)..."))

        if is_sync:
            self.stdout.write(self.style.WARNING("SYNC mode enabled. This may take a while..."))
        ...
        for i, norma in enumerate(normas, 1):
            ...
            try:
                if is_sync:
                    ...
                    self.stdout.write(self.style.NOTICE(f"  → Task dispatched: {task.id}"))
