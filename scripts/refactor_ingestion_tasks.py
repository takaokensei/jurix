...
GROUPS = {
    "task_support.py": ["_normalize_norma_tipo", "_configure_tesseract", "_invalidate_rag_cache", "_mark_norma_failed", "_resolve_norma_reference"],
    "core_tasks.py": ["cleanup_chat_attachments", "ingest_normas_task", "_process_norma_data", "bulk_ingest_normas_task", "ingest_sapl_corpus_task"],
    "download_tasks.py": ["download_pdf_task", "_sapl_payload_hash", "incremental_sync_sapl_task", "full_sync_sapl_task"],
...
def write_module(filename: str, names: list[str], nodes: dict[str, str]) -> None:
    ...
    (PKG / filename).write_text(imports + "\n" + extra + "\n" + body + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    (PKG / "tasks.py").write_text('''"""Stable public ingestion task API; implementations live by domain."""

from .consolidation_tasks import consolidate_norma_task
from .core_tasks import bulk_ingest_normas_task, cleanup_chat_attachments, ingest_normas_task, ingest_sapl_corpus_task
...
__all__ = [
    "bulk_ingest_normas_task", "cleanup_chat_attachments", "consolidate_norma_task", "download_pdf_task",
    "extract_entities_task", "full_sync_sapl_task", "generate_embedding_task", "incremental_sync_sapl_task",
    "ingest_normas_bulk_task", "ingest_normas_task", "ingest_sapl_corpus_task", "ocr_pdf_task", "segment_text_task",
]
''', encoding="utf-8")
    LEGACY.unlink()
